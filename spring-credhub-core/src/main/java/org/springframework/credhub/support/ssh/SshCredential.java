...
public class SshCredential extends KeyPairCredential {
	...
	/**
	 * Create an {@link SshCredential} from the provided public and private key. At least
	 * one of the key values must not be {@literal null}.
	 * @param publicKey the public key; may be {@literal null} only if
	 * {@literal privateKey} is not null
	 * @param privateKey the private key; may be {@literal null} only if
	 * {@literal publicKey} is not null
	 */
	...
	/**
	 * Get the fingerprint of the public key associated with the credential. This value
	 * can not be provided when creating an {@code SshCredential}, but may be provided by
