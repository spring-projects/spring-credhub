...
Request builders such as `PasswordParametersRequest` also accept an optional `metadata` map to store alongside a credential, as shown below; metadata requires CredHub server 2.6.0 or later.
