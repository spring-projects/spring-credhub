...
class ReactiveCertificateIntegrationTests extends ReactiveCredHubIntegrationTests {
	...
	void regenerateCertificateWithAllParameters() {
		...
		StepVerifier
			...
			.assertNext((response) -> assertThat(response.getName().getName()).isEqualTo(TEST_CERT_NAME.getName()))
			.verifyComplete();
	...
	void deleteCertificateVersion() {
		...
		StepVerifier.create(this.certificates.getVersions(certificateId.get())).expectNextCount(2).verifyComplete();

		StepVerifier.create(this.certificates.deleteVersion(certificateId.get(), credentialVersion0Id.get()))
			.assertNext((response) -> assertThat(response.getId()).isEqualTo(credentialVersion0Id.get()))
			.verifyComplete();
