...
class ReactiveCredentialIntegrationTests extends ReactiveCredHubIntegrationTests {
	...
	void writeCredential() {
		...
		StepVerifier.create(this.credentials.findByName(new SimpleCredentialName("/test")))
			.assertNext((response) -> assertThat(response).extracting("name")
				.extracting("name")
				.asString()
	...
	void writeCredentialWithMetadata() {
		...
		try {
			written = this.credentials
				.write(ValueCredentialRequest.builder()
		...
		StepVerifier.create(this.credentials.getById(written.getId(), ValueCredential.class))
			.assertNext((response) -> assertThat(response.getMetadata()).isEqualTo(metadata))
			.verifyComplete();
	...
	void overwriteCredentialV1() {
		...
		StepVerifier.create(this.credentials.write(ValueCredentialRequest.builder()
			.name(CREDENTIAL_NAME)
			.value("new-value")
			.mode(WriteMode.NO_OVERWRITE)
			.build())).assertNext((response) -> {
				assertThat(response.getName().getName()).isEqualTo(CREDENTIAL_NAME.getName());
				assertThat(response.getValue().getValue()).isEqualTo(CREDENTIAL_VALUE);
			}).verifyComplete();
	...
	void generateCredential() {
		...
		StepVerifier.create(this.credentials.generate(UserParametersRequest.builder()
			...
			.build(), UserCredential.class)).assertNext((response) -> {
				assertThat(response.getName().getName()).isEqualTo(CREDENTIAL_NAME.getName());
	...
	void generateCredentialWithMetadata() {
		...
		StepVerifier.create(this.credentials.regenerate(CREDENTIAL_NAME, UserCredential.class, updatedMetadata))
			.assertNext((response) -> assertThat(response.getMetadata()).isEqualTo(updatedMetadata))
			.verifyComplete();
	}
